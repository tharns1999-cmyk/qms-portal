...
const TaskReview = () => {
  ...
  const { tasks, dars, timeline, processWorkflow, currentUser, canDownloadDocument, documents } = useStore();
      ...
      <div className="w-[40%] flex flex-col bg-white border border-gray-200 rounded-xl overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-gray-50/50">
          ...
          <div className="bg-white p-5 rounded-xl border border-blue-100 shadow-sm">
            <div className="flex justify-between items-start mb-4">
               ...
               <span className="px-3 py-1 bg-blue-100 text-blue-700  text-xs font-bold rounded-full">{dar.type}</span>
            </div>
          ...
          <div className="flex flex-col gap-4">
             ...
             {darTimeline.map(tl => (
               <div key={tl.id} className={`flex flex-col ${tl.userId === currentUser.id ? 'items-end' : 'items-start'}`}>
                 ...
                 <div className={`p-3 rounded-2xl max-w-[90%] text-sm shadow-sm ${tl.userId === currentUser.id ? 'bg-blue-600 text-white rounded-tr-sm' : 'bg-white border border-gray-200 text-gray-800  rounded-tl-sm'}`}>
                    {tl.isChat ? (
                      <p>{tl.comment}</p>
                    ) : (
                      <div>
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold mb-1 bg-opacity-20 ${tl.action === 'Created' || tl.action === 'Resubmitted' ? 'bg-blue-100 text-blue-100' : 'bg-gray-100 text-gray-800 '}`}>
                           {tl.action}
                        </span>
                        <p>{tl.comment}</p>
                      </div>
        ...
        <div className="p-4 bg-white border-t border-gray-200 shadow-[0_-4px_10px_rgba(0,0,0,0.05)] z-10">
          <textarea
            ...
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-100 outline-none mb-3 bg-gray-50"
